...
func (r *CombinedResolver) ResolveValue(ref core.GlobalRef) (eval.Value, error) {
	// Case 0: ADT constructor factories ($adt.make_TypeName_CtorName)
	// These are generated by the elaborator for constructor calls like Some(x) or None.
	if ref.Module == "$adt" {
		if !strings.HasPrefix(ref.Name, "make_") {
			return nil, fmt.Errorf("invalid $adt factory name: %s", ref.Name)
		}
		parts := strings.SplitN(ref.Name[5:], "_", 2) // strip "make_", split TypeName_CtorName
		if len(parts) != 2 {
			return nil, fmt.Errorf("invalid $adt factory format: %s (expected make_Type_Ctor)", ref.Name)
		}
		typeName, ctorName := parts[0], parts[1]
		// Constructor should have been injected into env by injectADTConstructors
		if val, ok := r.Env.Get(ctorName); ok {
			return val, nil
		}
		// Fallback: search loaded modules directly
		for _, mod := range r.Modules {
			if mod == nil {
				continue
			}
			if foundType, exists := mod.Constructors[ctorName]; exists && foundType == typeName {
				if typeDecl, ok := mod.Types[typeName]; ok {
	...
	if ref.Module != "" {
		// Prefer qualified-name lookup to avoid collisions between modules
		// with same-named functions (e.g., std/string.length vs std/list.length).
			...
			for _, decl := range mod.Core.Decls {
				...
				if let, ok := decl.(*core.Let); ok {
					if let.Name == ref.Name {
						// Try unqualified lookup as fallback (single-module case)
						if val, ok := r.Env.Get(ref.Name); ok {
							return val, nil
						}
...
func (e *Executor) injectADTConstructors(evaluator *eval.CoreEvaluator) {
	...
	// Also inject ADT constructors from imported modules (e.g., Some/None from std/option).
	// This is needed when functions use Option/Result/other ADTs from stdlib.
	for _, mod := range e.modules {
		if mod == nil {
			continue
		}
		for ctorName, typeName := range mod.Constructors {
			if typeDecl, ok := mod.Types[typeName]; ok {
				if adt, ok := typeDecl.Definition.(*ast.AlgebraicType); ok {
					for _, ctor := range adt.Constructors {
						if ctor.Name == ctorName {
